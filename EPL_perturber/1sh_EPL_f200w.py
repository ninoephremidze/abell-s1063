...
def model_curved_fit_shapelet_sers(data,kappashear_params,source_params,likemask_list,indices):   
    ...
    for i in indices:
        if i == 1:
            lens_model_list_new = ['SHIFT','CURVED_ARC_SIS_MST','EPL']
        else:
...
maxepl = [1.,3.,xpp+0.7,ypp+0.2]
minepl = [0.,1.,xpp+0.1,ypp-0.3]

maxparam = maxlens1 + maxlens2 + maxshift + maxlens3 + maxshift + maxshape + maxepl
minparam = minlens1 + minlens2 + minshift + minlens3 + minshift + minshape + minepl
    ...
    kwargs_kapshe = [[{'alpha_x':0.,'alpha_y':0.},
                     ...
                     [{'alpha_x':params[7],'alpha_y':params[8]},
                      {'tangential_stretch': mut2, 'radial_stretch': mur2, 'curvature': cv2, 'direction': psi_ext2, 
                       'center_x': 0., 'center_y': 0.},
                      {'theta_E':params[18],'gamma':params[19],'e1':0.,'e2':0.,'center_x':params[21], 'center_y':params[20]}],
