...
# The priors that Cagan used for substructure search
...
# The priors I was using for a BIC nmax=8, two shapelet fit
...
maxlens3 = [2.,4.,0.1,0.4]
minlens3 = [0.5,1.,-0.6,0.1]

maxshift = [0.5,0.5]
minshift = [-0.5,-0.5]

maxshape = [2.0,0.4,0.4]
minshape = [0.4,-0.4,-0.4]

maxshape2 = [0.4,0.3,0.3]
minshape2 = [0.,-0.3,-0.3]
...
maxnfw = [10000.,500.,xpp+0.7,ypp+0.5]
minnfw = [0.,1.,xpp,ypp-0.5]
...
dresults = load_obj('POINT_MASS_NFW_FIT_POSA_with_10_8_PERTURBED_FIXmur=1_3_images_SHAPE_SHAPE' + 'nmax1' + str(nmax1) + 'nmax2' + str(nmax2))
...
labls_mask = ["$M_{200} / 10^7 M_{\odot}$", '$c_{200}$', "$x_p$ ['']", "$y_p$ ['']"]
...
tfig2,taxes2 = plt.subplots(N,N,figsize=(3*N,3*N))
...
for i in range(N):
    for j in range(N):
        taxes2[i,j].tick_params(which='major', length=10, width=5., direction='inout',pad=0.)
        if i == N-1:
            taxes2[i,j].set_xlabel(labls_mask[j])
            taxes2[i,j].xaxis.set_label_coords(0.5,-0.5)
            
        if j == 0:
            if i > 0:
                taxes2[i,j].set_ylabel(labls_mask[i])
                taxes2[i,j].yaxis.set_label_coords(-0.5,0.5)
...
# Assuming deltaPix is the pixel scale in arcseconds/pixel
...
circle_radius = 3  # Adjust the radius as needed
for row in range(3):
...
for col in range(3):
    ax[0,col].add_artist(lines.Line2D(x1, y1,color="white", linewidth=3*scal))
    ax[0,col].text(5,112, '1"', fontsize = fontsss*scal,color='White')

for col in range(3):
    ax[1,col].add_artist(lines.Line2D(x1, y1,color="white", linewidth=3*scal))
    ax[1,col].text(5,112, '1"', fontsize = fontsss*scal,color='White')

for col in range(3):
    ax[2,col].add_artist(lines.Line2D(x1, y1,color="Black", linewidth=3*scal))
    ax[2,col].text(5,112, '1"', fontsize = fontsss*scal,color='Black')
