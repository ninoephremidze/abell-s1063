...
def model_curved_fit_shapelet_sers(data,kappashear_params,source_params,likemask_list,indices):   
    ...
    for i in indices:
        if i == 1:
            lens_model_list_new = ['SHIFT','CURVED_ARC_SIS_MST','NFW_ELLIPSE']
        else:
...
maxnfw = [10000.,500.,xpp+0.75,ypp+0.15, 0.5, 0.5]
minnfw = [0.,1.,xpp+0.45,ypp-0.15, -0.5, -0.5]

maxparam = maxlens1 + maxlens2 + maxshift + maxlens3 + maxshift + maxshape + maxnfw
minparam = minlens1 + minlens2 + minshift + minlens3 + minshift + minshape + minnfw
    ...
    kwargs_kapshe = [[{'alpha_x':0.,'alpha_y':0.},
                     ...
                     [{'alpha_x':params[7],'alpha_y':params[8]},
                      {'tangential_stretch': mut2, 'radial_stretch': mur2, 'curvature': cv2, 'direction': psi_ext2, 
                       'center_x': 0., 'center_y': 0.},
                       {'Rs':Rs_angle, 'alpha_Rs':alpha_Rs, 'e1':params[22], 'e2':params[23], 'center_x':params[21], 'center_y':params[20]}],
...
labls_mask = ["$M_{200} / 10^7 M_{\odot}$", '$c_{200}$', "$x_p$ ['']", "$y_p$ ['']", "$e_1$", "$e_2$"]
